...
import { mergeGateAdmissionEvidenceBlocks } from './gate-admission-evidence.js'; import { mergeMachineGraphBlocks } from './machine-graph-metadata.js';
import { mergeParadigmBlocks } from './paradigm-metadata.js';
import { mergeSemanticHistoryBlocks } from './semantic-history-metadata.js';
const PROOF_GROUPS = ['contracts', 'refinements', 'invariants', 'termination', 'temporal', 'obligations', 'artifacts', 'assumptions'];

export function createParsedMetadata({ proofBlocks = [], paradigmBlocks = [], operationBlocks = [], semanticEditBlocks = [], semanticHistoryBlocks = [], conversionBlocks = [], constraintSpaceBlocks = [], decisionGraphBlocks = [], gateAdmissionEvidenceBlocks = [], dialectRegistryBlocks = [], interlinguaBlocks = [], resourceGraphBlocks = [], machineGraphBlocks = [], nativeSourceBlocks = [], packageManifestBlocks = [], canvasSurfaceBlocks = [], applicationSurfaceBlocks = [], runtimeCapabilityBlocks = [], targetProjectionTargets = [] } = {}) {
  const metadata = {};
  ...
  if (semanticHistoryBlocks.length) metadata.semanticHistory = mergeSemanticHistoryBlocks(semanticHistoryBlocks);
...
function mergeDecisionGraphBlocks(blocks) {
  ...
  return {
    ...
    panelProjectionIds: idsByKind(records, 'frontier.lang.decisionGraph.panelProjection'), rsiLoopIds: idsByKind(records, 'frontier.lang.decisionGraph.rsiLoop'), feedbackIds: idsByKind(records, 'frontier.lang.decisionGraph.improvementFeedback'),
    summary: {
      ...
      panelProjectionCount: sum(blocks, 'panelProjectionCount'), rsiLoopCount: sum(blocks, 'rsiLoopCount'), feedbackCount: sum(blocks, 'feedbackCount')
    },
...
function mergeResourceGraphBlocks(blocks) {
  ...
  return {
    ...
    escapeIds: blocks.flatMap((block) => ids(block.graph?.escapes)), coroutineScopeIds: blocks.flatMap((block) => ids(block.graph?.coroutineScopes)), greenThreadIds: blocks.flatMap((block) => ids(block.graph?.greenThreads)),
    ...
    summary: {
      ...
      escapeCount: sum(blocks, 'escapes'), coroutineScopeCount: sum(blocks, 'coroutineScopes'), greenThreadCount: sum(blocks, 'greenThreads'),
      ...
      lowLevelPrimitiveCount: sum(blocks, 'lowLevelPrimitives'), runtimeConcurrencyPrimitiveCount: sum(blocks, 'runtimeConcurrencyPrimitives'),
